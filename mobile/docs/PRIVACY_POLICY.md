...
Host this document at a public URL (for example GitHub Pages or your website) and
enter that URL in the Google Play Console **App content → Privacy policy** field and
in `mobile/src/content/legal.ts` (`PRIVACY_POLICY_URL`). The in-app text in
`mobile/src/content/legal.ts` must stay in sync with this file.
...
- A randomly generated device identifier is used for rate limiting and to validate
  temporary unlock sessions. It is not tied to your real-world identity.
- Recently searched usernames may be stored locally on your device to improve your
  experience. You can clear them by clearing app data.
...
Locally stored data stays on your device until you clear it. Server-side rate-limit and
unlock records are short-lived and automatically expire.

## Changes
We may update this policy. Continued use of the app after changes means you accept the
updated policy.
