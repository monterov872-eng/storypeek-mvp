...
import { fetchApiProfile } from '../services/apiProfile.js';
...
/** Flutter-friendly aliases under /api */
export function createApiStoriesRouter(config: AppConfig): Router {
  const router = Router();
  ...
  router.get('/profile/:username', profileSearchRateLimiter, async (req, res, next) => {
    try {
      ...
      const result = await fetchApiProfile(parsed.data, config);
      res.json(result);
    } catch (err) {
      next(err);
    }
      ...
      try {
        ...
        if (typeof req.query.highlight_json === 'string') {
          try {
            ...
            /* ignore malformed highlight_json */
          }
        }
